...
- **指数分析**（红利低波为默认入口）
  ...
  - 红利类：股息率与中国十年国债当前利差；海外股票不使用跨币种利差评分
  - PE / PB 与 PE 近 10 年分位
  - 红利、宽基、成长、技术采用不同诊断分项；缺少必需数据时不生成总分和评级；A-E 仅描述指标位置
  - 历史同评分统计要求估值来源及原始可用时间；缺少可核验的原时点数据时不输出胜率，不用当前盈利或分红回填历史
  - 规则化「走势解读」点评（走势本身 + 年线与布林关系）+ 一键复制小红书风格笔记文本
- **定投计划与 ETF 池**
  - 策略模拟：按目标比例或当前持仓市值比例，用同一组合、相同入金和费用比较周期定投与回撤加仓（如 3% / 5% 档距）；展示组合及逐只 ETF 的净收益、回撤、闲置现金与模拟买入记录
  - 现金流再平衡按投入后的目标金额补缺；初期建仓按绝对目标缺口分配，仍受交易执行约束
  - 模拟使用 ETF 自身共同日线区间，从空仓模拟新增投入，不回填真实持仓；少于 60 个共同交易日不计算，不足一年不展示年化。详细口径见 [策略模拟说明](docs/STRATEGY_SIMULATION.md)
  ...
  - 场外 ETF：交易记录可选场内/场外渠道；计划内可配置场外定投日程（频率、执行日、金额、估算净值），按周期自动写入交易记录，无需每日手填
