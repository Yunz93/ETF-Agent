...
跨市场个人股票工作台原型，覆盖 A 股、港股和美股。当前版本内置 60 只代表性股票，并支持按代码添加自定义标的；只展示后端成功获取的真实行情。
...
- **研究池**：搜索、市场筛选、行业筛选、估值状态筛选、同业对比（最多 4 只）
- **股票详情**：价格走势（目标价 / 成本标注）、估值区间、评分拆解、财报趋势、研究笔记与决策、事件与来源
- **导出**：Markdown 下载、浏览器打印 / 保存 PDF
- **提醒**：页面待办 + 可选浏览器 Notification；提醒历史本地保存
- **自定义标的**：按市场与代码加入自选 / 持仓（不限于内置 60 只）
