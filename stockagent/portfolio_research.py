...
def _run_portfolio_research(payload, *, history_loader=None, today=None):
    ...
    base = {"status": "insufficient_history" if reasons else "ready", "cutoff": cutoff,
            ...
            "configured_strategy_replay": {"status": "insufficient_history",
                "reasons": ["价格接口不提供原时点估值、情绪、溢价/价差、分红与初始持仓历史",
                            "可在研究页导入完整逐日历史包，复用实际交易规划进行固定参数时间留出比较"],
                "engine": "js/strategy-replay.js", "future_target_validated": False},
